...
本项目研究在四模态高斯混合 (GMM) 目标分布下，将**流匹配 (Flow Matching)** 模型作为重要性采样的**提议分布**，对比四种 FM 估计量与两种纯 RQMC 估计量在二阶矩与一个非线性"复杂被积函数"上的收敛阶。
...
| 被积 | FM-MC | FM-QMC | FM-ISMC | FM-ISQMC | Est. 1 | **Est. 2** |
|---|---|---|---|---|---|---|
...
│   ├── rqmc_baseline.py           # Estimator 1
│   ├── rqmc_advanced.py           # Estimator 2 (M1) + 其他探索方法
│   └── results_logi/
│       ├── fm_model_logi.pt
...
└── gmm30d/                        # 30D 实验
    ...
    ├── rqmc_baseline.py           # Estimator 1
    ├── rqmc_advanced.py           # Estimator 2 (M1)
...
### 评估 FM 与 Estimator 1（4 FM 方法 + Estimator 1 同时跑）
...
# series     ∈ {FM_MC, FM_QMC, FM_ISMC, FM_ISQMC, Estimator_1, Estimator_2}
```
