...
# ------------------------------
# Dataset (Banana distribution)
# ------------------------------

class BananaDataset(Dataset):
    def __init__(self, num_samples=5000, a=A, b=B, c=C):
        self.a = float(a)
        self.b = float(b)
        self.c = float(c)
    ...
    def _generate_samples(self, num_samples):
        z = np.random.randn(num_samples, 2).astype(np.float64)
        z1, z2 = z[:, 0], z[:, 1]
        x1 = z1
        x2 = self.a * (z1 ** 2) + self.c + self.b * z2
...
# ------------------------------
# True banana log-density (analytic)
# ------------------------------

LOG2PI = np.log(2.0 * np.pi)

def stdnorm_logpdf(x: np.ndarray) -> np.ndarray:
    return -0.5 * (x ** 2) - 0.5 * LOG2PI

def compute_true_banana_log_density(samples: np.ndarray, a=A, b=B, c=C) -> torch.Tensor:
    """
    Given samples X = (x1, x2), return log p_true(X) for the banana distribution:
      x1 = z1,
      x2 = a z1^2 + c + b z2,   (z ~ N(0, I))
    Density: p(x1, x2) = (1/|b|) φ(x1) φ((x2 - a x1^2 - c)/b)
    """
    x1 = samples[:, 0]
    x2 = samples[:, 1]
    u = (x2 - a * (x1 ** 2) - c) / b
    logp = stdnorm_logpdf(x1) + stdnorm_logpdf(u) - np.log(abs(b))
    return torch.from_numpy(logp.astype(np.float32))
...
class FlowMatchingOT(nn.Module):
    ...
    def forward(self, z):
        ...
        x0 = self.sample_from_base(z.shape)   # 从基分布采样 x0
    ...
    def sample(self, N, sampling_steps, integrator="heun"):
        ...
        self.model.eval()
        with torch.no_grad():
            x = self.sample_from_base((N, self.dim))  # 基分布起点
...
def compare_samples_with_truth(model,
                               ...
                               sampling_steps=32,
                               integrator="rk4",
                               save_path="results_logi/sample_compare_banana.png"):
    """
    采样模型 n_samples 个点，与真实 Banana 采样对比散点图。
    左：真实 Banana；右：模型采样
    """
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    # 真实 Banana 样本
    banana_dataset = BananaDataset(num_samples=n_samples)
    banana_samples = banana_dataset.data[:n_samples]
    ...
    axes[0].scatter(banana_samples[:, 0], banana_samples[:, 1], s=4, alpha=0.7)
    axes[0].set_title("True Banana Samples")
    axes[0].set_xlim(-4, 4); axes[0].set_ylim(-4, 4)

    axes[1].scatter(gen[:, 0], gen[:, 1], s=4, alpha=0.7)
    axes[1].set_title(f"Model Samples")
    axes[1].set_xlim(-4, 4); axes[1].set_ylim(-4, 4)

    for ax in axes:
        ax.set_xlabel("x", fontsize=17); ax.set_ylabel("y", fontsize=17)
...
def plot_distributions(base_data, target_data, generated_data=None, epoch=None, save_dir="results_logi"):
    os.makedirs(save_dir, exist_ok=True)
    fig, axes = plt.subplots(1, 3 if generated_data is not None else 2, figsize=(15, 5))

    axes[0].scatter(base_data[:, 0], base_data[:, 1], alpha=0.6, s=1)
    axes[0].set_title('Base Distribution (for reference)')
    axes[0].set_xlim(-4, 4); axes[0].set_ylim(-4, 4)

    axes[1].scatter(target_data[:, 0], target_data[:, 1], alpha=0.6, s=1)
    axes[1].set_title('Target: Banana')
    axes[1].set_xlim(-4, 4); axes[1].set_ylim(-4, 4)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"banana_flow_epoch_{epoch}.png"))
    plt.close(fig)
...
def train_model():
    ...
    # --- data ---
    banana_dataset = BananaDataset(num_samples=1000000)
    # 仅用于可视化对照：从基分布（这里是 logistic）和目标分布各取一部分
    base_ref = np.random.randn(1000, 2).astype(np.float32)  # 只是图里做参照（不严格等于 logistic）
    banana_ref = banana_dataset.data[:1000]
    ...
    for step in tqdm(range(1, steps + 1)):
        ...
        if step % vis_interval == 0:
            ...
            with torch.no_grad():
                ...
                plot_distributions(base_ref, banana_ref, generated_data=generated, epoch=step)
            model.train()

    # Save checkpoint after training
    model.eval()
    ckpt_path = os.path.join(save_dir, "fm_model_banana.pt")
...
def visualize_density(model, sampling_steps=64, save_path="results/density_banana.png"):
    grid_size = 100
    ...
    plt.title("Estimated Density from Flow Matching Model (Banana)")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.tight_layout()
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
def visualize_density_with_truth(model, sampling_steps=64,
                                 save_path="results/density_compare_banana.png",
                                 batch_size=256):
    """
    ...
    # ---- 解析真值 log p（Banana）----
    true_logp = compute_true_banana_log_density(coords.detach().cpu().numpy())
    true_logp = true_logp.numpy().reshape(grid_size, grid_size)

    # ---- 误差（概率域绝对误差；如需 log 域误差可改成 np.abs(logp_est - true_logp)）----
    ...
    axes[0].set_title("True Density (Banana)")
    axes[0].set_xlabel("x",fontsize =17); axes[0].set_ylabel("y",fontsize =17)
    fig.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04)

    # Estimated density
    im1 = axes[1].contourf(xx.numpy(), yy.numpy(), np.exp(logp_est), levels=100)
    axes[1].set_title("Estimated Density (Model)")
    axes[1].set_xlabel("x",fontsize =17); axes[1].set_ylabel("y",fontsize =17)
...
if __name__ == '__main__':
    ...
    # 用 Heun/RK4 做密度估计（反向积分）
    visualize_density_with_truth(model,
                                 sampling_steps=128,
                                 save_path="results_logi/density_compare_banana.pdf",
                                 batch_size=256)
    ...
    compare_samples_with_truth(model,
                               ...
                               sampling_steps=32,
                               integrator="heun",
                               save_path="results_logi/sample_compare_banana.pdf")

    print("Density plot saved at: results_logi/density_compare_banana.png")
