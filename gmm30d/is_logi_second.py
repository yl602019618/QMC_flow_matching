...
# ----------------------------
# Compute true 30D GMM *second* moment & log-prob
# ----------------------------

def compute_true_gmm_second_moment_30d(dim=30):
    """
    30D GMM 的真实二阶原始矩 m2[j] = E[X_j^2]：
    - 四个分量的均值在前两维为 (+/-2, +/-2)，其余维 0；
    - 协方差是 0.5 * I。
    对 mixture：E[X X^T] = Σ_k w_k (Σ_k + μ_k μ_k^T)，这里 w_k=0.25。
    对第 1,2 维：E[X_j^2] = 0.5 + E[μ_j^2] = 0.5 + 4 = 4.5
    其余维：E[X_j^2] = 0.5（因为 μ_j=0）
    """
    m2 = np.full(dim, 0.5, dtype=np.float32)
    m2[0] = 4.5
    m2[1] = 4.5
    return m2
...
# ----------------------------
# MC Estimation (proposal = model, 二阶矩)
# ----------------------------

def estimate_second_moment(model, p_values, true_mean=None, num_experiments=10):
    """
    估计 30D 的原始二阶矩 m2[j] = E[X_j^2]。
    这里仍然沿用字段名 true_mean/estimated_means，只是含义变成二阶矩。
    """
    model.eval()
    results = {
        'p_values': p_values,
        'sample_sizes': [],
        'estimated_means': [],   # 实际上是 estimated_second_moments
        'mse_errors': [],
        'bias_squared': [],
        'variance': [],
        'std_errors': [],
        'true_mean': true_mean   # 实际为 true_second_moment
    }
    ...
    with torch.no_grad():
        for p in tqdm(p_values):
            ...
            for _ in range(num_experiments):
                ...
                m2 = torch.mean(x ** 2, dim=0)                   # (30,) 二阶矩
...
# ----------------------------
# SNIS (proposal = model, target = true 30D GMM，二阶矩)
# ----------------------------
...
def estimate_second_moment_snis(model,
    ...
    估计原始二阶矩 m2[j] = E_p[X_j^2]，即 g(x) = x^2。
    """
    model.eval()
    ...
    results = {
        'p_values': p_values,
        'sample_sizes': [],
        'estimated_means': [],  # 实际为 estimated_second_moments
        'mse_errors': [],
        'bias_squared': [],
        'variance': [],
        'std_errors': [],
    ...
    for p in tqdm(p_values):
        ...
        for exp_id in range(num_experiments):
            ...
            # 5) 二阶矩 μ2 = E_p[X^2] ≈ Σ w_i x_i^2
            g = x ** 2
            mu_hat = torch.sum(w.unsqueeze(1) * g, dim=0)    # (30,)
...
def plot_snis_ess(snis_results, save_path="results30d/snis_ess.pdf"):
    ...
    plt.xlabel("Sample Size")
    plt.ylabel("ESS")
    plt.title("SNIS Effective Sample Size vs N (30D, Second Moment)")
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.5)
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
def main():
    ...
    true_second_moment = compute_true_gmm_second_moment_30d(dim=30)
    p_values = list(range(1, 15))  # N = 2^1 ... 2^14
    print("True Second Moment (30D):", true_second_moment)
    ...
    # SNIS（提议 = flow 模型，目标 = 30D 真 GMM，二阶矩）
    ...
    plot_mc_vs_qmc(mc_res, qmc_res,
                   ...
                   save_path="results30d/mc_qmc_snis_30d_second_moment.pdf")
    plot_snis_ess(snis_res_mc, save_path="results30d/snis_ess_mc_30d_second_moment.png")
    plot_snis_ess(snis_res_qmc, save_path="results30d/snis_ess_qmc_30d_second_moment.png")

    print("Evaluation complete (SECOND moment, 30D). Plots saved to results30d/.")
